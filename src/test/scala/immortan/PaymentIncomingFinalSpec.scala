...
class PaymentIncomingFinalSpec extends AnyFunSuite {
  test("Fulfill a single part incoming payment") {
    ...
    WAIT_UNTIL_TRUE(fsm.state.isInstanceOf[IncomingPaymentProcessor.Finalizing])
    WAIT_UNTIL_TRUE(
      fsm.data.asInstanceOf[IncomingRevealed].preimage == preimage
    )
    WAIT_UNTIL_TRUE(cm.getPreimageMemo.get(invoice.paymentHash).get == preimage)
    WAIT_UNTIL_TRUE(
      cm.payBag
        .getPaymentInfo(invoice.paymentHash)
        .get
  ...
  test("Fulfill multipart incoming payment") {
    ...
    WAIT_UNTIL_TRUE(fsm.state.isInstanceOf[IncomingPaymentProcessor.Receiving])
