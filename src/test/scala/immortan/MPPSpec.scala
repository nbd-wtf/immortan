...
class MPPSpec extends AnyFunSuite {
  ...
  test("Split between direct and non-direct channel") {
    ...
    val updateCAFromC = makeUpdate(5L, c, a, 1L.msat, 10, cltvDelta = CltvExpiryDelta(144), minHtlc = 10L.msat, maxHtlc = 100000000L.msat)
    val updateCAFromA = makeUpdate(5L, a, c, 1L.msat, 10, cltvDelta = CltvExpiryDelta(144), minHtlc = 10L.msat, maxHtlc = 100000000L.msat)
    ...
    val send = SendMultiPart(tag, Left(CltvExpiry(9)), SplitInfo(190000000L.msat, 190000000L.msat), routerConf,
      targetNodeId = a, None, None, totalFeeReserve = 1900000L.msat, allowedChans = cm.all.values.toSeq)
  ...
  test("Split after no route found on first attempt") {
    ...
    val send = SendMultiPart(tag, Left(CltvExpiry(9)), SplitInfo(600000L.msat, 600000L.msat), routerConf, targetNodeId = s, None, None,
  ...
  test("Correctly process fulfilled payment") {
    ...
    WAIT_UNTIL_RESULT {
      ...
      cm.opm process RemoteFulfill(UpdateAddHtlc(null, 1, MilliSatoshi(0L), hash, CltvExpiry(100L), p1.cmd.packetAndSecrets.packet, null), preimage)
      cm.opm process RemoteFulfill(UpdateAddHtlc(null, 1, MilliSatoshi(0L), hash, CltvExpiry(100L), p2.cmd.packetAndSecrets.packet, null), preimage)
      cm.opm process RemoteFulfill(UpdateAddHtlc(null, 1, MilliSatoshi(0L), hash, CltvExpiry(100L), p3.cmd.packetAndSecrets.packet, null), preimage)
  ...
  test("Handle multiple competing payments") {
    ...
    val send1 = SendMultiPart(tag1, Left(CltvExpiry(9)), SplitInfo(300000L.msat, 300000L.msat), routerConf, targetNodeId = s,
      None, None, totalFeeReserve = 6000L.msat, allowedChans = cm.all.values.toSeq, assistedEdges = Set(edgeDSFromD))
  ...
  test("Smaller part takes disproportionally larger fee from reserve") {
    ...
    val send = SendMultiPart(tag, Left(CltvExpiry(9)), SplitInfo(600000L.msat, 600000L.msat), routerConf, targetNodeId = d,
      None, None, totalFeeReserve = 6000L.msat, allowedChans = cm.all.values.toSeq)
    ...
    cm.pf process makeUpdate(3L, b, d, 5950L.msat, 100, cltvDelta = CltvExpiryDelta(144), minHtlc = 10L.msat, maxHtlc = 500000L.msat)
