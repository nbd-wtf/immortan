...
class MPPSpec extends AnyFunSuite {
  test("Gradually reduce failed-at-amount") {
    ...
    assert(
      data1
        .withFailuresReduced(stamp + LNParams.failedChanRecoveryMsec.toLong / 2)
        .chanFailedAtAmount
        .head
        ._2
  ...
  test("Split after no route found on first attempt") {
    ...
    WAIT_UNTIL_TRUE(cm.opm.data.payments(tag).data.usedFee == MilliSatoshi(24L))
  ...
  test("Fail on local timeout") {
    ...
    WAIT_UNTIL_TRUE {
      ...
      // Our only channel is offline, sender FSM awaits for it to become operational
      assert(
        parts1.head.asInstanceOf[WaitForChanOnline].amount == send.split.myPart
      )
      parts1.size == 1
    }
