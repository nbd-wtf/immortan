...
class ElectrumClientSpec extends TestKitBaseClass with AnyFunSuiteLike {
  ...
  test("get header") {
    ...
    val GetHeaderResponse(_, header) = probe.expectMsgType[GetHeaderResponse]
  ...
  test("scripthash subscription") {
    ...
    val ScriptHashSubscriptionResponse(_, status) = probe1.expectMsgType[ScriptHashSubscriptionResponse]
    assert(status != "")
  }

  test("get scripthash history") {
    probe.send(client, GetScriptHashHistory(scriptHash))
    val GetScriptHashHistoryResponse(_, history) = probe.expectMsgType[GetScriptHashHistoryResponse]
  ...
  test("list script unspents") {
    ...
    val ScriptHashListUnspentResponse(_, unspents) = probe.expectMsgType[ScriptHashListUnspentResponse]
