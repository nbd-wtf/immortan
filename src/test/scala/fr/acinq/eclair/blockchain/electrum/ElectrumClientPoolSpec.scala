...
class ElectrumClientPoolSpec extends TestKitBaseClass with AnyFunSuiteLike {
  ...
  test("scripthash subscription") {
    ...
    val ScriptHashSubscriptionResponse(_, status) = probe1.expectMsgType[ScriptHashSubscriptionResponse](timeout)
    assert(status != "")
  }
  ...
  test("list script unspents") {
    ...
    val ScriptHashListUnspentResponse(_, unspents) = probe.expectMsgType[ScriptHashListUnspentResponse](timeout)
