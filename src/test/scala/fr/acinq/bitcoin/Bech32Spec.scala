...
class Bech32Spec extends FunSuite {
  ...
  test("parse testnet taproot address") {
    val address = "tb1pqqqqp399et2xygdj5xreqhjjvcmzhxw4aywxecjdzew6hylgvsesf3hn0c"
    val (hrp, witnessVersion, data) = Bech32.decodeWitnessAddress(address)
    val OP_1 :: OP_PUSHDATA(bin, _) :: Nil = addressToPublicKeyScript(address, Block.TestnetGenesisBlock.hash)
