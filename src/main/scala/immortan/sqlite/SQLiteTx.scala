...
class SQLiteTx(val db: DBInterface) {
  def listRecentTxs(limit: Int): RichCursor =
    db.select(TxTable.selectRecentSql, Array(limit.toString))

  def listAllDescriptions: Map[String, TxDescription] =
    db.select(TxTable.selectRecentSql, Array(10000.toString))
      .iterable { rc =>
  ...
  def addSearchableTransaction(search: String, txid: ByteVector32): Unit = {
    db.change(TxTable.newVirtualSql, Array(search.toLowerCase, txid.toHex))
  }
  ...
  def updDescription(description: TxDescription, txid: ByteVector32): Unit =
    db txWrap {
      db.change(
        TxTable.updateDescriptionSql,
        Array(description.toJson.compactPrint, txid.toHex)
      )
  ...
  def removeByPub(xPub: ExtendedPublicKey): Unit = {
    db.change(TxTable.killByPubSql, Array(xPub.publicKey.toString))
    ChannelMaster.next(ChannelMaster.txDbStream)
  }
