...
import immortan.channel.CommitmentOutput._
...
object Transactions {
  ...
  case class ClaimHtlcSuccessTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  case class ClaimHtlcTimeoutTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  case class ClaimAnchorOutputTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  ...
  case class HtlcPenaltyTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  case class ClosingTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  ...
  def commitTxFeeMsat(
    ...
    weight2feeMsat(spec.commitTxFeerate, weight) + anchorsCost
  }
