...
import scoin.ln.addressToPublicKeyScript
import utest._
import scodec.bits._

/** Created by fabrice on 19/04/17.
  */
object Bech32Spec extends TestSuite {
  val tests = Tests {
    ...
    test("decode lnurl string") {
      ...
      inputs.map { case (encodedLnurl, expected) =>
    ...
    test("parse testnet taproot address") {
      val address =
        "tb1pqqqqp399et2xygdj5xreqhjjvcmzhxw4aywxecjdzew6hylgvsesf3hn0c"
      val (hrp, witnessVersion, data) = Bech32.decodeWitnessAddress(address)
      val OP_1 :: OP_PUSHDATA(bin, _) :: Nil =
        addressToPublicKeyScript(address, Block.TestnetGenesisBlock.hash)
      assert(Bech32.encodeWitnessAddress(hrp, 1, bin) == address)
      assert(witnessVersion == 1)
      assert(data == bin)
    }
  }
}
