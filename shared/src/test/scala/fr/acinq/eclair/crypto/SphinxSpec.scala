package scoin.ln.crypto

import scoin.ByteVector32
import scoin.Crypto.{PrivateKey, PublicKey}
import scoin.ln.crypto.Sphinx.RouteBlinding.BlindedRoute
import scoin.ln._
import scoin.ln.{CltvExpiry, MilliSatoshi, UInt64, randomKey}
import utest._
...
/** Created by fabrice on 10/01/17.
  */
object SphinxSpec extends TestSuite {
  import Sphinx._
  import SphinxTestHelpers._
  ...
  val tests = Tests {
    ...
    test(
      "create payment packet with variable-size payloads filling the onion"
    ) {
      val PacketAndSecrets(onion, sharedSecrets) = create(
        sessionKey,
        1300,
    ...
    test("concatenate blinded routes (reference test vector)") {
      ...
      val (blindedRouteStart, payloadsStart) = {
        ...
        (
          RouteBlinding.create(sessionKey, publicKeys.take(2), payloads),
          payloads
        )
      }
